...
const CreateAlbum = () => {
  ...
  let handleSubmit = async (e) => {
    ...
    try {
      ...
      if (albumThumbnail) {
        ...
        albumData.append("cloud_name", "dihmrthrw");

        //! upload album thumbnail to the clouudinary
        let response = await fetch("https://api.cloudinary.com/v1_1/dihmrthrw/upload",
          {
            method: "POST",
            body: albumData
          }
      ...
      //! Step-2: Second We will upload song thumbnail  on cloudinary
      ...
      await Promise.all(
        songDetails.map(async (song, index) => {
          ...
          if (song.songThumbnail) {
            //! Add the song thumbnail and song file which is .mp3 to the formData()=> API
            let songFileData = new FormData();
            songFileData.append("file", song.songThumbnail);
            songFileData.append("upload_preset", "music_album");
            songFileData.append("cloud_name", "dihmrthrw");

            //! upload songthumbnail to cloudinary fetch ->API
            let response = await fetch("https://api.cloudinary.com/v1_1/dihmrthrw/upload", {
              method: "POST",
              body: songFileData
            });
            ...
            console.log("Song Thumbail:", uploadedSongThumbnailUrl)
          }

          //! upload the song file logic
  ...
  return (
    <section className="w-full flex flex-col justify-center items- text-white p-6">
      {/* //? Starting of Add Album Code */}
      <article className="w-full bg-gray-800 rounded-t-md">
        <header>
