  ...
  let handleSubmit = async (e) => {
    ...
    try {
      if (password === confirmPassword) {
        ...
        let registredUser = await createUserWithEmailAndPassword(__AUTH, email, password);
        console.log(registredUser);

        //! send email verification
        sendEmailVerification(registredUser.user);

        //! update profile name and photo url which is not updated default
        updateProfile(registredUser.user,{
          displayName:username,
          photoURL:"https://tse1.mm.bing.net/th?id=OIP.F0cHWZXD0VdlYDhZN3Uy7AHaHa&pid=Api&P=0&h=180"
        })
        toast.success(`Email verification has been sent to your registered ${email}`);
        toast.success("User has beeen registred successfully");
        navigate("/auth/login");
      }
      else {
      ...
      <article className="w-[30%] bg-gray-700 p-5 rounded-xl ">
        ...
        <form onSubmit={handleSubmit}>
          <div className='flex flex-col mb-1 p-3'>
            ...
            <input type="text" id='username' placeholder='Enter your name' className='outline-none border border-gray-500 p-2 rounded-lg' name="username" value={username}
              onChange={handleInputChange} />
          </div>
          <div className='flex flex-col mb-1 p-3'>
            <label htmlFor="email" className='font-semibold text-lg mb-1'>Email</label>
            <input type="email" id='email' placeholder='Enter your email' className='outline-none border border-gray-500 p-2 rounded-lg' name="email" value={email}
              onChange={handleInputChange} />
          </div>
          <div className='flex flex-col mb-1 p-3 relative'>
            <label htmlFor="password " className='font-semibold text-lg mb-1'>Password</label>
            <input type={showPassword1 ? "text" : "password"} id='password' placeholder='Enter your password' className='outline-none border border-gray-500 p-2 rounded-lg' name="password" value={password}
              onChange={handleInputChange} />
          ...
          <div className='flex flex-col mb-1 p-3 relative'>
            <label htmlFor="confirmPassword" className='font-semibold text-lg mb-1'>Confirm Password</label>
            <input type={showPassword2 ? "text" : "password"} id='confirmPassword' placeholder='Confirm your password' className='outline-none border border-gray-500 p-2 rounded-lg' name="confirmPassword" value={confirmPassword}
              onChange={handleInputChange} />
