            ...
            <form onSubmit={handleSubmit}>
              <div className='flex flex-col mb-1 p-3 relative'>
                ...
                <input type="email" id='email' placeholder='Enter your email' className='outline-none border border-gray-500 p-2 rounded-lg' name="email" value={email}
                  ...
                  <span className='absolute bottom-[25px] right-[20px] text-lg cursor-pointer'><FaUser/>
                  </span>
              </div>
              <div className='flex flex-col mb-1 p-3 relative'>
                <label htmlFor="password " className='font-semibold text-lg mb-1'>Password</label>
                <input type={showPassword ? "text" : "password"} id='password' placeholder='Enter your password' className='outline-none border border-gray-500 p-2 rounded-lg' name="password" value={password}
                  onChange={handleInputChange} />
                <span onClick={togglePassword} className='absolute bottom-[25px] right-[20px] cursor-pointer text-lg'>
                  {showPassword ? <IoEye/> : <IoEyeOff/>}
                </span>
              </div>
              
              <div className='flex flex-col mb-1 p-3'>
                <button className="bg-blue-600 py-2 text-lg rounded-lg cursor-pointer font-semibold hover:bg-blue-700" >Login</button>
              </div>
